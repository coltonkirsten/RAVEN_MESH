"""Nexus Agent (Isolated) — RAVEN Mesh actor that runs Claude Code inside Docker.

A sibling of nodes/nexus_agent. Same mesh-side wiring (registers with Core,
boots a control loopback server, boots an inspector UI) — but instead of
spawning the host's `claude` binary, it spawns a docker container running
the nexus_agent_isolated image. The MCP bridge is BAKED INTO the image; the
agent has no view of host code.

Defaults:
  - Inspector UI: 127.0.0.1:8806
  - Control:      127.0.0.1:8816 (reached from container as host.docker.internal:8816)
  - Image:        nexus_agent_isolated:latest
  - Volume:       nexus_agent_isolated_ledger
"""
...
DEFAULT_IMAGE = os.environ.get("NEXUS_AGENT_ISOLATED_IMAGE", "nexus_agent_isolated:latest")
DEFAULT_VOLUME = os.environ.get("NEXUS_AGENT_ISOLATED_VOLUME", "nexus_agent_isolated_ledger")
...
class AgentRuntime:
    ...
    def __init__(
        ...
        self.image = image
        self.ledger_volume = ledger_volume
    ...
    async def handle_inbox(self, env: dict) -> dict | None:
        async with self._lock:
            ...
            try:
                result = await run_claude_in_container(
                    message=text,
                    system_prompt=self.system_prompt(),
                    image=self.image,
                    ledger_volume=self.ledger_volume,
                    control_port=self.inspector.control_port,
                    control_token=self.control_token,
                    model=self.model,
                    session_id=self.session_id,
                    on_event=on_event,
                )
                # If we tried to resume a session that the container can no
                # longer find (e.g. the volume was wiped), drop it and retry
                # with a fresh session. Better than dying.
                if (
                    result.is_error
                    and self.session_id
                    and "No conversation found" in (result.result_text or "")
                ):
                    log.warning("session %s lost; retrying without --resume", self.session_id)
                ...
                log.exception("docker/claude run failed")
...
def make_control_app(rt: AgentRuntime) -> web.Application:
    """Same control surface as nexus_agent — bridge calls these over HTTP."""
    app = web.Application()
...
async def run(
    ...
    rt = AgentRuntime(
        node=node, model=model, inspector=inspector,
        image=image, ledger_volume=ledger_volume,
    )
    ...
    # Bind on 0.0.0.0 so the container can reach us via host.docker.internal.
    # Token + path-traversal-safe handlers are still in place.
    control_site = web.TCPSite(control_runner, "0.0.0.0", control_port)
    await control_site.start()
    log.info("[%s] control server: http://0.0.0.0:%s (token=%s…)",
    ...
    print(f"[{node_id}] nexus_agent_isolated ready. inspector: http://{inspector_host}:{inspector_port}",
          flush=True)
...
def main() -> int:
    ...
    p.add_argument("--node-id", default="nexus_agent_isolated")
    ...
    p.add_argument("--model",
                   default=os.environ.get("NEXUS_AGENT_ISOLATED_MODEL",
                                          os.environ.get("NEXUS_AGENT_MODEL", "claude-sonnet-4-6")))
    p.add_argument("--inspector-host",
                   default=os.environ.get("NEXUS_AGENT_ISOLATED_HOST", "127.0.0.1"))
    p.add_argument("--inspector-port", type=int,
                   default=int(os.environ.get("NEXUS_AGENT_ISOLATED_PORT", "8806")))
    p.add_argument("--control-port", type=int,
                   default=int(os.environ.get("NEXUS_AGENT_ISOLATED_CONTROL_PORT", "8816")))
    p.add_argument("--image", default=DEFAULT_IMAGE)
    p.add_argument("--ledger-volume", default=DEFAULT_VOLUME)
    args = p.parse_args()
